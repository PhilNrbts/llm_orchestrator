...
class WorkflowExecutor:
    ...
    def execute_workflow(self, workflow_name: str, workflow: Workflow, params: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            if isinstance(workflow.params, list):
                ...
                workflow_params_dict = {name: param.dict() for name, param in workflow.params.items()}
            
            self.current_params = validate_workflow_params(workflow_name, {"params": workflow.params}, params)
        ...
        with Progress(
            ...
            for i, step in enumerate(workflow.steps, 1):
                task = progress.add_task(f"Executing step {i}/{len(workflow.steps)}: {step.name}", total=None)
                ...
                try:
                    ...
                    if step.gate:
                        ...
                        if not gate_result:
                            console.print(f"🛑 Workflow stopped at gate: {step.name}", style="yellow")
                            break
                        continue
                    
                    # Resolve dynamic inputs
                    resolved_inputs = self._resolve_inputs(step.inputs, self.current_params, self.step_outputs)
                    
                    # Execute the step (simulated for now)
                    ...
                    if step.on_failure == "abort_chain":
                        console.print("🛑 Aborting workflow due to step failure", style="red")
                        return {"error": f"Step '{step.name}' failed: {str(e)}", "completed_steps": self.step_outputs}
                    elif step.on_failure == "continue":
                        console.print("⚠️  Continuing workflow despite step failure", style="yellow")
                        self.step_outputs[step.name] = {"error": str(e)}
                        continue
        
        console.print(f"🎉 Workflow '{workflow_name}' completed successfully!", style="bold green")
        return self.step_outputs
    
    def _resolve_inputs(self, step_inputs: Dict[str, Any], params: Dict[str, Any], previous_outputs: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        for key, value in step_inputs.items():
            if isinstance(value, str):
                resolved[key] = self._resolve_template_string(value, params, previous_outputs)
            elif isinstance(value, dict):
                resolved[key] = self._resolve_inputs(value, params, previous_outputs)
            elif isinstance(value, list):
                resolved[key] = [
                    self._resolve_template_string(item, params, previous_outputs) if isinstance(item, str) else item
                    for item in value
                ]
            else:
                resolved[key] = value
    ...
    def _resolve_template_string(self, template: str, params: Dict[str, Any], previous_outputs: Dict[str, Any]) -> str:
        """
    ...
    def _lookup_value(self, path: str, params: Dict[str, Any], previous_outputs: Dict[str, Any]) -> Any:
        """
        Look up a value from params or previous step outputs.
        
        Args:
            path: Dot-notation path like 'params.query' or 'steps.initial_answer.output'
            params: Workflow parameters
            previous_outputs: Outputs from previous steps
        ...
        if parts[0] == 'params':
            ...
            if len(parts) >= 2:
                ...
                for part in parts[2:]:
                    if isinstance(result, dict) and part in result:
                        ...
                        if part == 'output' and isinstance(result, (str, int, float, bool)):
                            return result
            ...
            raise KeyError(f"Unknown path prefix: {parts[0]}. Use 'params.' or 'steps.'")
    
    def _execute_step(self, step: Step, resolved_inputs: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        if step.tool in self.tools:
            ...
            console.print(f"⚠️  Unknown tool '{step.tool}', using simulation", style="yellow")
            return {"output": f"[Simulated output from unknown tool: {step.tool}]", "inputs": resolved_inputs}
    ...
    def _handle_scrutiny_gate(self, step: Step) -> bool:
        ...
        resolved_prompt = self._resolve_template_string(prompt_text, self.current_params, self.step_outputs)
        ...
        while True:
            response = console.input("\n[bold yellow]Approve (y/n)?[/bold yellow] ").lower().strip()

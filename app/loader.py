...
def load_config(path: str = "config.yaml") -> Config:
    ...
    try:
        ...
        console.print("Please ensure config.yaml exists in the current directory.", style="yellow")
        raise SystemExit(1)
        ...
        console.print("Configuration validation failed with the following errors:", style="yellow")
        ...
        console.print(f"❌ Unexpected error loading configuration: {str(e)}", style="red")
        raise SystemExit(1)


def validate_workflow_params(workflow_name: str, workflow_config: Dict[str, Any], provided_params: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    if isinstance(workflow_params, list):
        ...
        required_params = [name for name, param in workflow_params.items() if param.required]
        optional_params = {name: param.default for name, param in workflow_params.items() if not param.required}
    
    # Check for missing required parameters
    missing_params = [param for param in required_params if param not in provided_params]
    if missing_params:
        raise ValueError(f"Missing required parameters for workflow '{workflow_name}': {missing_params}")

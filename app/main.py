...
def run(prompt, parallel_models, steps, password_option):
    ...
    _password = get_password(password_option)
...
def list_workflows():
    ...
    try:
        ...
        console.print(
            Panel.fit("[bold blue]Available Workflows[/bold blue]", border_style="blue")
        )

        for name, info in workflows.items():
            # Create a table for each workflow
            table = Table(
                title=f"🔧 {name}", show_header=True, header_style="bold magenta"
            )
...
def run_workflow(workflow_name, params, password_option):
    ...
    try:
        ...
        for param in params:
            if "=" not in param:
                console.print(
                    f"[bold red]Error:[/bold red] Invalid parameter format '{param}'. Use key=value format."
                )
                return
        ...
        required_params = [
            p["name"] for p in workflow_info["parameters"] if p.get("required", True)
        ]
        missing_params = [p for p in required_params if p not in parsed_params]

        if missing_params:
            console.print(
                f"[bold red]Error:[/bold red] Missing required parameters: {', '.join(missing_params)}"
            )
            console.print("\nRequired parameters:")
        ...
        if os.path.exists("memory.db"):
            ...
            console.print(
                "🔍 Use 'llm-cli workflow inspect' to view detailed execution history"
            )
...
def inspect_workflow(run_id, recent, all):
    ...
    try:
        if not os.path.exists("memory.db"):
            console.print(
                "[yellow]No memory database found. Run a workflow first.[/yellow]"
            )
            return
        ...
        if all:
            ...
            stats_table.add_row(
                "Database Size", f"{stats['database_size_bytes']} bytes"
            )
        ...
        with sqlite3.connect("memory.db") as conn:
            ...
            if recent or not run_id:
                ...
                if not result:
                    console.print(
                        "[yellow]No workflow runs found in memory database.[/yellow]"
                    )
                    return
                ...
                if not results:
                    console.print(
                        f"[yellow]No workflow run found matching '{run_id}'.[/yellow]"
                    )
                    return

                if len(results) > 1:
                    console.print(
                        f"[yellow]Multiple workflows found matching '{run_id}':[/yellow]"
                    )
            ...
            console.print(
                Panel.fit(
                    "[bold green]🔍 Workflow Execution Details[/bold green]",
                    border_style="green",
                )
            )
            ...
            info_table.add_row(
                "Workflow ID",
                workflow_id[-20:] + "..." if len(workflow_id) > 20 else workflow_id,
            )
            ...
            memory_entries = memory_store.retrieve(
                workflow_id=workflow_id, order_by="timestamp ASC"
            )
            ...
            if memory_entries:
                ...
                for entry in memory_entries:
                    ...
                    entries_table.add_row(
                        entry["step_name"], entry["classification"], content, timestamp
                    )

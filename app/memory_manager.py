...
class MemoryManager:
    ...
    def __init__(
        self, memory_store: Optional[MemoryStore] = None, db_path: str = "memory.db"
    ):
        """
    ...
    def start_workflow(self, workflow_name: str, initial_params: Dict[str, Any]) -> str:
        ...
        if "user_prompt" in initial_params:
            self.memory_store.add_entry(
                ...
                metadata={"workflow_name": workflow_name, "all_params": initial_params},
            )
    ...
    def _resolve_memory_need(
        self, need: str, workflow_history: List[Dict[str, Any]]
    ) -> Optional[str]:
        """
        ...
        if tool_output_match:
            ...
            return self.memory_store.retrieve_step_output(
                self.current_workflow_id, step_name
            )

...
from app.key_management import derive_key, VAULT_FILE_PATH, PASSWORD_SALT
...
def handle_existing_vault():
    ...
    try:
        ...
        console.print(f"[bold red]Error:[/bold red] Could not rename the old vault: {e}")
        return False
...
def parse_api_keys(text_block: str) -> dict:
    ...
    for line in lines:
        ...
        if not match:
            raise ValueError(f"Invalid format for line: '{line}'. Expected format: KEY_NAME=key_value")
            
        key_name = match.group('name')
        key_value = match.group('key')
        
        if not key_name.endswith("_API_KEY"):
            raise ValueError(f"Invalid key name: '{key_name}'. All keys must end with '_API_KEY'.")
...
def init_vault():
    ...
    try:
        ...
        detected_key_names = ", ".join([k.replace('_API_KEY', '') for k in api_keys.keys()])
        if not click.confirm(f"\n[bold green]Success![/bold green] {len(api_keys)} keys were detected: {detected_key_names}.\nIs that correct?"):
            console.print("Aborting vault setup.")
            return

        password = click.prompt("Enter a master password for the vault", hide_input=True, confirmation_prompt=True)
        ...
        console.print("[italic]Note: The main application will automatically load keys from this vault when you run it.[/italic]")

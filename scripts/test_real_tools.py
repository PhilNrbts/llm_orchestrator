...
def test_real_tools():
    ...
    try:
        ...
        console.print("  • 🤖 ModelCallTool - Real LLM API calls with fallback simulation")
        console.print("  • 🔄 ParallelQueryTool - Concurrent execution of multiple queries")
        ...
        result1 = engine.run("sequential_elaboration", {
        ...
        if 'initial_answer' in result1:
            ...
            if 'simulated' in output:
                console.print(f"✅ ModelCallTool executed (simulation mode: {output['simulated']})")
            else:
                console.print("✅ ModelCallTool executed with real API")
        
        # Test parallel_query tool
        console.print("\n🧪 [bold]Testing ParallelQueryTool[/bold]")
        result2 = engine.run("parallel_summarizer", {
        ...
        if 'parallel_execution' in result2:
            ...
            console.print(f"✅ ParallelQueryTool executed {query_count} queries ({successful} successful)")
        ...
        console.print("  • BaseTool: Abstract interface ensuring consistent tool structure")
        ...
        console.print("The workflow engine now executes real tools instead of simulations!")

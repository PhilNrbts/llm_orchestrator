...
from app.key_management import derive_key, PASSWORD_SALT, VAULT_FILE_PATH
...
def display_export_commands():
    ...
    if not os.path.exists(VAULT_FILE_PATH):
        console.print(f"[bold red]Error:[/bold red] Main vault `[cyan]{VAULT_FILE_PATH}[/cyan]` not found.")
        return
    ...
    try:
        ...
        password = Prompt.ask("Enter your master password to load the keys", password=True)
        ...
        with open(script_filename, "w") as f:
            if is_windows:
                for line in decrypted_data.strip().split('\n'):
                    if line:
                        f.write(f"$env:{line.replace('=', "='", 1)}'\n")
            else:
                f.write("#!/bin/bash\n")
                for line in decrypted_data.strip().split('\n'):
                    if line:
                        f.write(f"export {line.replace('=', "='", 1)}'\n")
        ...
        console.print("For security, [bold red]delete the script immediately after[/bold red] using it:")
...
def view_vault_flow():
    ...
    try:
        ...
        password = Prompt.ask("Enter your master password to view the vault", password=True)
        key = derive_key(password, PASSWORD_SALT)
        cipher = Fernet(key)
        ...
        table = Table(title="Vault Contents", show_header=True, header_style="bold magenta")
        ...
        for line in lines:
            ...
            if match:
                ...
                redacted_value = f"{key_value[:4]}...{key_value[-4:]}" if len(key_value) > 8 else key_value
...
def change_password_flow():
    ...
    try:
        ...
        try:
            ...
            console.print("[bold red]Invalid password. Password change failed.[/bold red]")
            return
        ...
        new_password_confirm = Prompt.ask("Confirm your NEW master password", password=True)
...
def delete_vault_flow():
    ...
    choice_str = Prompt.ask("Enter the number of the vault to delete (or 'q' to quit)", default="q")
    ...
    if not click.confirm(f"Are you sure you want to permanently delete '[bold red]{vault_to_delete}[/bold red]'?"):
        console.print("Deletion cancelled.")
        return

    try:
        os.remove(vault_to_delete)
        console.print(f"Vault '[bold green]{vault_to_delete}[/bold green]' has been deleted.")
    except OSError as e:
...
def set_main_vault_flow():
    ...
    choice_str = Prompt.ask("Enter the number of the vault to set as main (or 'q' to quit)", default="q")
    if choice_str.lower() == 'q':
        return
    ...
    if os.path.exists("vault.enc"):
        ...
        console.print(f"Current main vault renamed to `[cyan]{new_name_for_old_main}[/cyan]`.")

    os.rename(vault_to_promote, "vault.enc")
    console.print(f"Successfully set `[bold green]{vault_to_promote}[/bold green]` as the new main `vault.enc`.")

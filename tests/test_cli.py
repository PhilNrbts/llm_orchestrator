...
def mock_all_clients(monkeypatch):
    ...
    async def fake_parallel_query(prompt, api_keys, model_config):
        return {model: f"Response from {model} for '{prompt}'" for model in model_config.keys()}
...
def test_run_parallel(runner, mock_vault):
    ...
    result = runner.invoke(cli, [
        'run', '-p', 'test prompt', '--model', 'gemini',
        '--model', 'anthropic-haiku', '--password', password
    ])
...
def test_interactive_chat_model_switching(runner, mock_vault, monkeypatch):
    ...
    def mock_ask(prompt_text, **kwargs):
        ...
        if "Hello" in prompt_text: return "Hello"
        if "Continue" in prompt_text: return "Continue"
        if "Last message" in prompt_text: return "Last message"
